...
│   └── test_app.py            # Pytest suite for the Flask dashboard (crontab mocked)
├── .github/workflows/
...
- **`state.json`**: Persisted state including `prices` (keyed by route label `"ORIGIN-DEST DATE"`, each containing `price`, `previous_price` (the price it was compared against for that run's alert, `null` on the first check — persisted rather than re-derived from `history` so the dashboard's displayed change always matches what was actually alerted on, even after retention trimming prunes older history points), `updated`, a `details` object with the cheapest offer's airlines/stops/duration plus `alternatives`/`nonstop_price`/`price_level`/`typical_price_range` (simple routes) or a `legs` array of per-leg price/airlines/stops/times (multi-leg routes — see the `flight_monitor.py` bullet), and a `history` array), `api_calls` (keyed by `YYYY-MM`), `last_run` timestamp, and `flex_scans` (keyed by the same `"ORIGIN-DEST DATE"` route label, each holding the most recent flexible-date scan: `base_date`, `days`, per-date `results`, and the `cheapest` entry). Written atomically via a temp file + `os.replace` so a crash mid-write can't corrupt it. Every read-modify-write cycle (in `flight_monitor.py`'s `main()`/`run_scan()`/`run_trim()`/`sync_and_persist_account_quota()`, and `app.py`'s `DELETE /api/routes/<label>`) is also wrapped in `flight_monitor.state_lock()`, a cross-process `fcntl.flock` on `state.json.lock` (POSIX-only, gitignored, local only) held for the whole span, not just the save — without it, a route deleted from the dashboard mid-run could reappear because the monitor's in-memory copy (loaded before the delete, held for the run's full duration) overwrites the disk afterward. `app.py` imports `state_lock()` (and `route_label()`/`route_search_cost()`/`_is_legs_route()`) from `flight_monitor.py` rather than keeping its own copies, so the two processes can't drift out of sync.
...
A multi-leg (multi-city) route's `prices`/`flex_scans` entry is keyed the same way, just with the chain-joined label (see the `routes.json` bullet above) — e.g. a JFK-HEL, HEL-BER, BER-JFK itinerary keys as `"JFK-HEL-BER-JFK 2026-09-15"`. Its `details` shape differs from a simple route's: `price` is the **sum** of each leg's own cheapest one-way price (not one combined multi-city fare — see the `flight_monitor.py` bullet for why), `airlines`/`stops`/`total_duration` are aggregated across legs, there's no `alternatives`/`nonstop_price`/`price_level`/`typical_price_range` (those describe alternates for a single flight search, which doesn't map onto a sum of independent legs), and `details.legs` is an array of each leg's own `origin`/`destination`/`date`/`price`/`airlines`/`stops`/`departure_time`/`arrival_time`/`total_duration`.
...
- **Two entry points**: `flight_monitor.py` (cron job) and `app.py` (web server), run as separate processes. They share `state.json` (coordinated by `state_lock()` — see the `state.json` bullet), and `app.py` imports a few small route-label/scheduling helpers from `flight_monitor.py` to avoid duplicating them, but neither process depends on the other being up.
...
Run `.venv/bin/python flight_monitor.py --scan` (optionally `--days N`) to sweep each route's `departure_date ± N` days and report the cheapest date. On-demand only; costs one search per date for a simple route, `route_search_cost()` (N per leg) per date for a multi-leg route, and respects `MONTHLY_CALL_CAP`.
