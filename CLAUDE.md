...
- **`tests/test_app.py`**: Pytest suite for `app.py` using Flask's test client — route/state validation (`validate_routes`, `normalize_route`), the `DELETE /api/routes/<label>` data-deletion endpoint, dashboard rendering, and cron schedule logic (`build_cron_block`, `current_schedule`, `publish_schedule`). Never touches the real crontab: any code path that would shell out to `crontab` has `subprocess.run` mocked.
